...
function statusChip(status: FeeStatus, t: (s: string) => string) {
  const map: Record<FeeStatus, { label: string; bg: string; fg: string }> = {
    COMPLETE: { label: t('Fees complete'), bg: '#dcfce7', fg: '#16a34a' },
...
export default function StudentFeesModal({
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true)
      setData(await getStudentFees(studentId))
    } catch {
        ...
        <View style={s.sheet}>
          ...
          <View style={s.header}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, flex: 1 }}>
              <Ionicons name="wallet-outline" size={20} color="#F03E2F" />
              <View style={{ flex: 1 }}>
                <Text style={s.title}>{t('School Fees')}</Text>
            ...
            <ScrollView keyboardShouldPersistTaps="handled" style={{ maxHeight: '88%' }}>
              ...
              <View style={s.summaryRow}>
                <View style={s.summaryCard}>
                  <Text style={s.summaryLabel}>{t('Total fee')}</Text>
                  <Text style={s.summaryValue}>{formatXAF(data.due)}</Text>
                </View>
                <View style={s.summaryCard}>
              ...
              <View style={s.detailBox}>
                ...
                <View style={s.detailRow}>
                  <Text style={s.detailLabel}>{t('Class')}</Text>
                  <Text style={s.detailValue}>{data.student.classLevel}</Text>
                </View>
                <View style={s.detailRow}>
                ...
                <View style={[s.detailRow, { borderBottomWidth: 0 }]}>
                  <Text style={s.detailLabel}>{t('Last payment')}</Text>
                  <Text style={s.detailValue}>{data.payments.length ? fmtDate(data.payments[data.payments.length - 1].paidOn) : '—'}</Text>
                </View>
              </View>
              {chip && (
              ...
              {data.due > 0 && data.balance > 0 ? (
                ...
                <Text style={[s.hint, { color: '#16a34a', fontWeight: '600' }]}>{t('Fees fully paid for this session. ✓')}</Text>
              )}
            </ScrollView>
          ) : null}
        </View>
