...
export default function TermsPage() {
  ...
  const handleEndYear = async () => {
    ...
    try {
      ...
      const msg = result.decisionsSet > 0
        ? `${result.session} ${t('closed')} · ${result.decisionsSet} ${t(result.decisionsSet !== 1 ? 'report cards' : 'report card')} ${t('updated with PASS/REPEAT decisions')}`
        : `${result.session} ${t('closed')}`
      showToast(msg, 'success')
      setShowEndYear(false)
      fetchTerms()
    } catch (err: unknown) {
      const e2 = err as { response?: { data?: { message?: string } } }
      showToast(e2.response?.data?.message || t('Failed to end academic year'), 'error')
    } finally {
      setEndingYear(false)
    }
  }
  ...
  const handleStartYear = async () => {
    ...
    for (const td of nyTerms) {
      if (!td.startDate || !td.endDate) { setNyError(t('Please fill in all start and end dates.')); return }
    }
    setStartingYear(true)
    try {
      const result = await startNewAcademicYearApi(nySession, nyTerms)
      showToast(`${result.session} ${t('started — first')} ${tt('term', 'semester')} ${t('set as current')}`, 'success')
      setShowNewYear(false)
      fetchTerms()
    } catch (err: unknown) {
      const e2 = err as { response?: { data?: { message?: string } } }
      setNyError(e2.response?.data?.message || t('Failed to start new academic year'))
    } finally {
    ...
    <div>
      ...
      {hasActiveTerm ? (
        <div className="bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-800 rounded-xl p-4 mb-6 flex items-center justify-between gap-4">
          ...
          >
            {t('End Academic Year')} <ArrowRight size={14} />
          </button>
        </div>
      ) : (
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-blue-800 dark:text-blue-300">
                {t('No active academic year')}
              </p>
              <p className="text-xs text-blue-700 dark:text-blue-400 mt-0.5">
                {t('Start a new academic year to activate a session and allow report cards, fees, and promotions.')}
              </p>
            </div>
          </div>
          <button
      ...
      {allSessions.length > 1 && (
        <div className="flex items-center gap-2 mb-4 flex-wrap">
          <span className="text-xs text-muted-foreground">{t('All sessions:')}</span>
          {allSessions.map((s) => (
          ...
          <div className="bg-card rounded-2xl w-full max-w-sm p-6 border border-transparent dark:border-border">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-foreground text-lg flex items-center gap-2">
                <AlertTriangle size={18} className="text-amber-500" /> {t('End Academic Year')}
              </h3>
              <button onClick={() => setShowEndYear(false)} className="text-muted-foreground hover:text-foreground"><X size={20} /></button>
            </div>
            <p className="text-sm text-muted-foreground mb-3">
              {t('You are closing')} <strong className="text-foreground">{currentSession}</strong>. {t('This will:')}
            </p>
            ...
            <div className="flex gap-3">
              ...
              <button type="button" onClick={handleEndYear} disabled={endingYear}
                ...
                {endingYear ? t('Ending year...') : t('Yes, end the year')}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/60 dark:bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-card rounded-2xl w-full max-w-md max-h-[90vh] flex flex-col border border-transparent dark:border-border">
            <div className="flex items-center justify-between p-6 pb-4 border-b border-border">
              <h3 className="font-semibold text-foreground text-lg flex items-center gap-2">
                <PartyPopper size={18} className="text-blue-500" /> {t('Start New Academic Year')}
              </h3>
            ...
            <div className="overflow-y-auto flex-1 p-6 space-y-5">
              ...
              <div>
                <label className="block text-xs font-medium text-foreground mb-1">{t('Session Name')} <span className="text-destructive">*</span></label>
                <input type="text" placeholder="2025/2026"
                ...
                <p className="text-xs text-muted-foreground mt-1">{t('Use the format YYYY/YYYY (e.g. 2025/2026)')}</p>
              </div>
                ...
                <div className="space-y-3">
                  {nyTerms.map((td, i) => (
                    <div key={i} className="border border-border rounded-xl p-3 space-y-2">
                      ...
                      {i === 0 && (
                        <p className="text-xs text-primary">{tt('↑ This term will be set as current automatically', '↑ This semester will be set as current automatically')}</p>
                      )}
                    </div>
                  ))}
                </div>
            ...
            <div className="p-6 pt-4 border-t border-border flex gap-3">
              ...
              <button type="button" onClick={handleStartYear} disabled={startingYear || !nySession.trim()}
                ...
                {startingYear ? t('Starting...') : `${t('Start')} ${nySession || t('new year')}`}
              </button>
            </div>
          </div>
        </div>
