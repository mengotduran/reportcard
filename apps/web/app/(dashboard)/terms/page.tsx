...
export default function TermsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(e.response?.data?.message || 'Failed to save term')
    } finally {
      setSaving(false)
    }
  }
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Manage academic terms and sessions</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-card rounded-xl border border-border text-center py-12">
          ...
          <p className="text-muted-foreground text-sm">No terms yet. Add your first term.</p>
        </div>
      ) : (
        ...
        <div className="fixed inset-0 bg-black/60 dark:bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-card rounded-2xl w-full max-w-sm p-6 border border-transparent dark:border-border">
            <div className="flex items-center justify-between mb-5">
              <h3 className="font-semibold text-foreground text-lg">{editingId ? 'Edit Term' : 'Add Term'}</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              <div>
                <label className="block text-xs font-medium text-foreground dark:text-foreground mb-1">Term Name</label>
              ...
              <div>
                <label className="block text-xs font-medium text-foreground dark:text-foreground mb-1">Session</label>
              ...
              <div>
                <label className="block text-xs font-medium text-foreground dark:text-foreground mb-1">Start Date</label>
                <input type="date"
              ...
              <div>
                <label className="block text-xs font-medium text-foreground dark:text-foreground mb-1">End Date</label>
                <input type="date"
