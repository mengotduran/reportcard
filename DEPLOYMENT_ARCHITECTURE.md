...
- **Not verified**: whether `net stop ReportCardSystem` inside `PrepareToInstall` actually stops a real Windows Service when run for real — Wine's Service Control Manager is already documented (section 16) as a known-incomplete reimplementation, so this specific behavior needs real Windows hardware to confirm, same caveat as the rest of the Windows service path. The Pascal Script itself compiling and running without error under Wine is confirmed; whether the `net stop` command it issues has a real service to find is not, in this environment.
