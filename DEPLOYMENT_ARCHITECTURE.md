...
> Status: **BUILT, not yet field-deployed.** The Cloud setup is live. The offline/SQLite setup (single-device and multi-device — same build, see section 3) has all 7 build-order steps done as of 2026-06-24, its schema kept in sync with Postgres as of 2026-07-19 (section 8a) — Linux fully verified on real hardware, the Windows installer verified end-to-end under Wine, Windows/macOS auto-start service registration written but still unverified on real Windows/Mac hardware. No offline install has shipped to an actual school yet. See "Build order" (section 11) for the precise per-step status.
>
...
### 8a. First real sync since launch — 2026-07-19, and a real drift bug found

The "ongoing cost" above was theoretical until now: the SQLite schema hadn't been touched since 2026-06-29, while the Postgres schema had moved through 12 more migrations (departments/secondary sections, resit scores, the school stamp image, official-header letterhead text, marks-entry-mode + its audit log, student birth details, teacher department placement).

**A real, pre-existing drift bug was found while doing this, unrelated to the 12 new migrations**: `Department`/`ClassLevel.departmentId` and `School.website`/`School.authorizationNumber` were already declared in `prisma/sqlite/schema.prisma` — hand-copied over during an earlier sync pass — but **no migration for them had ever actually been generated**. Confirmed empirically with `prisma migrate diff --from-migrations prisma/sqlite/migrations --to-schema prisma/sqlite/schema.prisma --script` (rather than manually eyeballing both files, which is exactly how this kind of gap hides). Any offline install built before this fix would have crashed the first time that code path ran — the identical failure mode documented in section 2a for the Postgres/production side, just never triggered here yet because no offline build has shipped to a real school.

**Fix**: one consolidated migration, `20260719000000_sync_departments_marks_mode_birth_details`, covering both the old drift and everything added since — generated via the same `migrate diff --script` command, then hand-fixed for the known unquoted-JSON-default bug (three more instances: `ExcelTemplate.classLevels`, `School.coverImages`, `User.departments`). Registered in `src/config/sqliteMigrations.ts` per the existing pattern.

**Verification** (the `migrate diff` tool itself doesn't confirm the SQL is actually valid or that the app works against it — separately confirmed both):
- `prisma migrate diff ... --exit-code` after writing the migration: zero remaining difference between the migration history and `schema.prisma`.
- Deleted the local disposable `dev.db` and reapplied all 5 migrations from scratch, in order — proves a **fresh** install (the only case that matters; no real offline install has this drift baked in yet) migrates cleanly, not just an already-drifted one.
- Regenerated the SQLite Prisma Client and exercised every new/fixed field end-to-end against the fresh database directly (`Department` + `ClassLevel.departmentId` relation, `MarksEntryModeChange`, `User.departments` as a working JS array, `Student.dateOfBirth`/`placeOfBirth`, `ReportEntry.resitScore`, `School.marksEntryMode`/`stamp`/`officialLeftTextEn`) — all round-tripped correctly.
- Ran the actual `bundle.mjs` esbuild step (not just the schema/migration tooling) and grepped the output for the new migration's name and content — confirmed esbuild's `.sql` text-loader picked it up and inlined it into the real bundle a packaged executable would ship with.
- **Not done**: a full SEA-executable + launcher isolated-directory run (the "verified in full isolation" discipline used in sections 12/14) — this change only touches schema/migration content, not the packaging mechanics those sections cover, and those were already proven separately. Worth a full rebuild+isolated-run before this next ships to an actual school, same as any offline-build change.
- `src/scripts/sqliteSmokeTest.ts` was extended to also exercise all of the above, matching its existing coverage style — but **could not be re-run via its documented `npx ts-node` invocation**, because `sqliteMigrations.ts`'s `.sql` imports only resolve through esbuild's build-time text loader, not plain `ts-node`. Worth fixing (e.g. a ts-node loader for `.sql`, or documenting the bundle-first requirement explicitly) so the smoke test's own instructions stay accurate — not fixed here, out of scope for a schema sync.
...
## 18. Updating an existing offline install — ✅ DONE 2026-07-19

The explicit requirement: a school with an offline install already running needs to receive future fixes/features **without losing their data, even across a schema change**, and without any internet access on their end (the whole install is offline by design — there's no auto-update-over-the-internet mechanism, and building one is out of scope here). Concretely, this means: rebuild, carry the new installer to the school on a USB drive same as the first install (section 7), run it **on top of** the existing install.

**Two things this relies on were already true by construction, not new work:**
- **Data already lives outside the installed application entirely.** The SQLite file + uploaded images + the launcher's persisted `JWT_SECRET`/config live in `%APPDATA%`/`~/Library/Application Support`/XDG data dir (section 4, section 14) — completely separate from `{app}` (Program Files) / the release directory that an update actually overwrites. Re-running the installer never touches this.
- **Schema migrations already apply automatically and safely on top of existing data.** `SQLITE_MIGRATIONS` (`src/config/sqliteMigrations.ts`) is an append-only list; `runSqliteMigrations()` tracks what's already applied in `_app_migrations` and only runs what's new, every startup, unconditionally. An update that bundles new migrations just... works, the moment the updated binary next starts — this is exactly the same mechanism section 8a's schema sync relies on, just from the other direction (a school receiving that sync as a shipped update instead of it being applied by hand here).

**What was NOT already safe, found by actually thinking through what happens to a *running* service during an update, and fixed:**
- **Windows**: a running Windows Service holds its `.exe`/`.dll` files open — Setup's `[Files]` copy step would fail outright trying to overwrite `reportcard-api.exe`/`reportcard-launcher.exe`/`web\node.exe` while the old version is still running as the service. Added a `[Code]` `PrepareToInstall` hook to `reportcard-system.iss` that runs `net stop ReportCardSystem` before file extraction begins (Inno Setup's `[Run]` entries only fire *after* installation — too late for this). Tolerant of the service not existing yet (first install) or already being stopped — a non-zero exit from `net stop` is deliberately ignored, not treated as an installer error.
- **Windows, part 2**: `install-service.js`'s `alreadyinstalled` handler (fires on every update, since the service is already registered from the previous version) only logged a message and added firewall rules — it never actually restarted the service. Combined with the fix above, an update would have stopped the service, copied in new files, and then left it stopped until the machine's next reboot. Fixed: `alreadyinstalled` now calls `svc.start()`, same as the fresh-install path.
- **Linux**: `linux-install.sh` (the same script used for both first install and updates) ended with `systemctl --user start reportcard.service` — a no-op on an already-running unit, so a re-run during an update would leave the *old* process running old code indefinitely (Linux, unlike Windows, happily lets you overwrite a running binary's file on disk; the already-running process just keeps using its old file handle until it actually exits). Changed to `systemctl --user restart`, which correctly starts-if-stopped or stop-then-starts-if-running either way.
- **macOS**: already correct as originally written — `macos-install.sh` already does `launchctl unload ... || true` before `launchctl load -w`, which is the same stop-then-start pattern, tolerant of "wasn't loaded yet." No change needed.

**Verified**:
- Re-compiled `reportcard-system.iss` (with the new `[Code]` section) end-to-end via the same Wine/`ISCC.exe` path as section 16 — compiles clean, produces `ReportCardSystemSetup.exe`.
- **Update 2026-07-19, confirmed on real Windows hardware**: `net stop` alone was **not enough** — every real update attempt failed with `DeleteFile failed; code 5. Access is denied` on `reportcard-launcher.exe`. `net stop` only guarantees the SCM-registered top-level process is asked to stop; it does not guarantee node-windows' service wrapper actually kills the full process tree it spawned (`reportcard-launcher.exe --service`, and in turn `reportcard-api.exe` and web's `node.exe` as *its* children), and does nothing at all for a separate instance launched via the desktop shortcut rather than the service. Fixed by force-killing the app's own executables by name afterward (`taskkill /F /IM ... /T`, tree-kill so each process's children go with it) plus a 2s `Sleep` for the OS to actually release the file handles, both in `PrepareToInstall`. This is the first real-hardware confirmation of anything in the Windows service path predating this — and it confirmed the *original* single-`net-stop` approach was insufficient, exactly the kind of gap "written but unverified" was flagging.
