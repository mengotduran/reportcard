...
- There's already early, *unfinished and currently disconnected* groundwork for this sitting in the Postgres schema migration history: `SyncTombstone` / `SyncToken` / `SyncCursor` tables and `School.accessExpiresAt` / `restrictionNote` columns (added to the database via a migration, but with **no corresponding Prisma models in `schema.prisma` and no app code using them**). Don't assume this means sync is partially built — it isn't. Treat it as a placeholder for when this phase is picked up.
